...
namespace structures
{
	...
	inline ArrayList<T>& ArrayList<T>::operator=(const ArrayList<T>& other)
	{
		// test identity
		//		zrus stare pole
		//		vytvorim si nove pole ako kopiu pola *other.array_
		//		priradim size_
		if (this != &other)
		{
			delete array_;
	...
	{
		// for i od 0 do size_ - 1;
		//		ak je prvok v poli na indexe i rovnaky ako prvok z parametra;
		//			tak vratim i;

		// vrat -1;

		for (int i = 0; i < size_; i++) {	// n*
			if ((*array_)[i] == data) {		// O(1)
				return i;					// ----
			}								// O(n)
		}
	...
	{
		//Array resize
		Array<T>* newArray_ = new Array<T>(2 * array_->size());
		Array<T>::copy(*array_, 0, *newArray_, 0, size_);
		delete array_;
		array_ = newArray_;
	}
