...
namespace structures
{
	...
	{
		// musi byt PreOrder (popripade LevelOrder)
	...
	{
		if (this->isRoot()) {
			throw std::logic_error("There is not a brother!");
		}
	...
	inline size_t TreeNode<T>::sizeOfSubtree()
	{
		...
		// spracuj mnozinu synov a zavolaj opat prehliadku
		for (int i = 0; i < degree(); i++) {
			TreeNode<T>* son = getSon(i);
			if (son != nullptr) {
