...
namespace structures
{
	...
	void PriorityQueueLimitedSortedArrayList<T>::push(const int priority, const T & data)
	{
		if (this->list_->size() != capacity_) {
			...
			throw std::logic_error("Priority queue limited sorted array list is out of capacity!");
		}
	}
	...
	{
		PriorityQueueItem<T>* newPQI = nullptr;
		PriorityQueueSortedArrayList<T>::push(priority, data);

		if (this->list_->size() > capacity_) {
			//newPQI = new PriorityQueueItem<T>(priority, data);
			//PriorityQueueSortedArrayList<T>::push(priority, data);
			newPQI = (*this->list_)[0];
	...
	inline int PriorityQueueLimitedSortedArrayList<T>::minPriority() const
	{
		if (this->list_->isEmpty()) {
			throw std::logic_error("Priority queue limited sorted array list is empty!");
		}
