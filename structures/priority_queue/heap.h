...
namespace structures
{
	...
	void Heap<T>::push(const int priority, const T& data)
	{
		...
		while (currentIndex > 0 && (*this->list_)[currentIndex]->getPriority() < (*this->list_)[parentIndex]->getPriority()) {
			// vymen data current a parent
	...
	T Heap<T>::pop()
	{
		...
		while (sonIndex != -1 && (*this->list_)[currentIndex]->getPriority() > (*this->list_)[sonIndex]->getPriority()) {
			// vymen data current a parent
	...
	inline int Heap<T>::getGreaterSonIndex(const int index)
	{
		...
		PriorityQueueItem<T>* leftSon = leftIndex < this->list_->size() ? (*this->list_)[leftIndex] : nullptr;
		PriorityQueueItem<T>* rightSon = rightIndex < this->list_->size() ? (*this->list_)[rightIndex] : nullptr;
		...
		if (leftSon != nullptr && rightSon != nullptr) {
			return leftSon->getPriority() < rightSon->getPriority() ? leftIndex : rightIndex;
		}
		else {
			return leftIndex;
		}
