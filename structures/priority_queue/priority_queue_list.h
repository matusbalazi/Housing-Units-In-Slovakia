...
namespace structures
{
	...
	inline int PriorityQueueList<T>::indexOfPeek() const
	{
		...
		//
		// zapamataj si naj index a naj prioritu. inicializuj ich na -1, max
		// aktualny index = 0
		// 
		// pomocou cyklu foreach
		//		ak (aktualna priorita < naj priorita)
		//				naj priorita = aktualna priorita
		//				naj index = aktualny index
		// aktualny index++
		//
		// return naj index

		if (list_->isEmpty()) {
			throw std::logic_error("PriorityQueueList is empty!");
		}
		int bestIndex = -1;
	...
	{
		// zistim si index of peek
		// z listu zavolam remove at (iop) a zapamatam si item
		// zapamatam si data itemu
		// zrusim item
		// vratim data
