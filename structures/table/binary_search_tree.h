...
namespace structures
{
	...
	inline BinarySearchTree<K, T>& BinarySearchTree<K, T>::operator=(const BinarySearchTree<K, T>& other)
	{
		if (this != &other)
		{
			...
			// vytvor si preorder iterator "na zaciatok" (iterCurrent)
			// vytvor si preorder iterator "na koniec" (iterEnd)
	...
	inline T& BinarySearchTree<K, T>::operator[](const K key)
	{
		...
		if (found) {
			...
			throw std::out_of_range("Nenasli sa data s danym klucom!");
		}
	}
	...
	{
		// 1_ ak je strom prazdny, tak node sa stane korenom (replaceRoot) a hotovo
		// 2_ pomocou findBSTNode najdi otca (hladam kluc vkladaneho vrchola a found musi vratit false), kluc ziskam ako node->accessData()->getKey()
		// 3_ ak je found true -> duplicita, vrat false a hotovo
		// 4_ ak je found false -> zarad node ako laveho/praveho syna najdenemu otcovi (setLeft(node), setRight(node)) a vrat true a hotovo
