...
namespace structures
{
	...
	template<typename K, typename T>
	inline T& UnsortedSequenceTable<K, T>::operator[](const int index)
	{
		if (index >= 0 && index < this->list_->size()) {
			return (*this->list_)[index]->accessData();
		}
		else {
			throw std::out_of_range("Nastala chyba!");
		}
	}

	template<typename K, typename T>
	inline const T UnsortedSequenceTable<K, T>::operator[](const int index) const
	{
		if (index >= 0 && index < this->list_->size()) {
			return (*this->list_)[index]->accessData();
		}
